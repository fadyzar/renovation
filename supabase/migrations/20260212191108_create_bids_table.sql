/*
  ...
  1. New Tables
    - `bids`
      ...
      - `status` (text) - Bid status: sent, viewed, accepted, rejected
  ...
  2. Security
    ...
    - Contractors can update their own bids (only when status is 'sent')
...
-- Contractors can update their own bids when status is 'sent'
CREATE POLICY "Contractors can update own sent bids"
  ON bids FOR UPDATE
  TO authenticated
  USING (
    contractor_id = auth.uid()
